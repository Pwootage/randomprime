...
impl<'a> GcDiscLookupExtensions<'a> for structs::GcDisc<'a> {
    ...
    fn add_file(&mut self, path: &str, file: structs::FstEntryFile<'a>) -> Result<(), String> {
        ...
        let file_name = split.next().ok_or_else(|| "".to_owned())?;

        let new_entry =
            structs::FstEntry::File(Cow::Owned(CString::new(file_name).unwrap()), file, None);
    ...
    pub fn decompress(&self) -> Cow<'a, [u8]> {
        if self.is_compressed {
            ...
            Decompress::new(false)
                .decompress(&reader, &mut output, FlushDecompress::Finish)
                .unwrap();
